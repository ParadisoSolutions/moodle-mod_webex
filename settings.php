...
if ($ADMIN->fulltree) {
    ...
    $name = 'webex/enablemeeting';
    $visiblename = get_string('enablemeeting','webex');
    $description = get_string('enablemeetingdesc','webex');
    $settings->add(new admin_setting_configcheckbox($name, $visiblename, $description, 0, 1, 0));
    
    $name = 'webex/enabletraining';
    $visiblename = get_string('enabletraining','webex');
    $description = get_string('enabletrainingdesc','webex');
    $settings->add(new admin_setting_configcheckbox($name, $visiblename, $description, '1', 1, 0));
    
    $name = 'webex/enablewebexdebug';
    $visiblename = get_string('enablewebexdebug','webex');
    $description = get_string('enablewebexdebugdesc','webex');
    $settings->add(new admin_setting_configcheckbox($name, $visiblename, $description, '1', 1, 0));
    
    //--- general settings -----------------------------------------------------------------------------------
    //$webExID, $password, $siteID, $partnerID, $siteURL
    
    $settings->add(new admin_setting_configtext('webex/webexid',
        "Webex Id", "webexid",""));
    $settings->add(new admin_setting_configtext('webex/webexpassword',
        "Webex Passsword","webexpassword" , "" )); 
    
    $settings->add(new admin_setting_configtext('webex/siteid',
        "Webex Site Id", "siteid",""));
    $settings->add(new admin_setting_configtext('webex/partnerid',
        "Webex Partner Id", "partnerid",""));
    
    
    $settings->add(new admin_setting_configtext('webex/siteurl',
        "Webex Site URL","siteurl" , "" )); 
    
    $settings->add(new admin_setting_configtext('webex/fromemailaddress',
        "From Email Address", "fromemailaddress",$CFG->noreplyaddress));
    $settings->add(new admin_setting_configtext('webex/fromemailname',
        "From Email Name","fromemailname" , "noreply" ));
